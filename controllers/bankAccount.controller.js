...
exports.allBanckAcounts = async (req, res) => {
    ...
    try {
        ...
        let bankData = await BankAcountModel.find(filter)
            ...
            .limit(10);
        if (!bankData)
        return res.status(404).send({statusCode:404,message:"We don't found any accounts"});
        let bankCount = await BankAcountModel.count();
...
exports.banckAcountDelete = async (req, res) => {
    let data = { _id: req.body.id }
    try {
        let bankData = await BankAcountModel.update(data, { isDeleted: req.body.isDeleted })
        ...
        const page = filter.page;
        delete filter.page;
        let bankDataAll = await BankAcountModel.find(filter)
            .sort("-creationDate")
            .skip(_skip)
            .limit(10);
        if (!bankDataAll)
