...
exports.adminChangePassword = async(req, res) => {
    try {
        if (!req.body.id || !req.body.password)
            return res.send({statusCode:405,message:'Please enter required fields.'});

        const _user = await UserModel.findOne({ _id: req.body.id, role: 'admin' });
        if (!_user)
            return res.send({statusCode:405,message:"No user found with this data"});

        // Generate Password
        const password = await passwordService.generatePassword(req.body.password, _user._id);
        if (_.isNil(password) || password == false)
            return res.send({statusCode:405,message:"error Happened while generate new password"});
        ...
        return res.send({statusCode:405,message:"Error happened while update user data"});
    }
}
...
exports.adminGetUserById = async(req, res) => {
    try {
        if (!req.body.id)
            return res.send({statusCode:405,message:"No user valid with this data"});
        let _user = await UserModel.findOne({ _id: req.body.id, role: 'admin' }, '-password -userToken');
        return res.send({statusCode:200,message:"success",data:_user});
    } catch (err) {
...
exports.adminUpdateUser = async(req, res) => {
    try {
        ...
        if (_mobile && String(_mobile._id) != String(_user._id))
            return res.send({statusCode:405,message:"this mobile number already registered for another user before ."});

        let _res = await UserModel.findOneAndUpdate({ _id: req.body.id }, { $set: data }, { new: true });
        if (!_res)
            return res.send({statusCode:405,message:"Can not update this user,try in another time."});
...
exports.adminCreateUser = async(req, res) => {
    try {
        ...
        if (_.isNil(_country))
            return res.send({statusCode:405,message:"Please enter valid country"});
        ...
        return res.send({statusCode:404,message:"error Happened ",data: err || "error happened" });
    }
}
