...
exports.adminChangePassword = async(req, res) => {
    try {
        if (!req.body.id || !req.body.password)
            return res.status(405).send({ statusCode: 405,message:'Please enter required fields.'});

        const _user = await UserModel.findOne({ _id: req.body.id, role: 'admin' });
        if (!_user)
            return res.status(405).send({ statusCode: 405,message:"No user found with this data"});

        // Generate Password
        const password = await passwordService.generatePassword(req.body.password, _user._id);
        if (_.isNil(password) || password == false)
            return res.status(405).send({ statusCode: 405,message:"error Happened while generate new password"});
        ...
        return res.status(405).send({ statusCode: 405,message:"Error happened while update user data"});
    }
}
...
exports.adminGetUserById = async(req, res) => {
    try {
        if (!req.body.id)
            return res.status(405).send({ statusCode: 405,message:"No user valid with this data"});
...
exports.adminUpdateUser = async(req, res) => {
    try {
        ...
        if (_mobile && String(_mobile._id) != String(_user._id))
            return res.status(405).send({ statusCode: 405,message:"this mobile number already registered for another user before ."});
...
exports.adminCreateUser = async(req, res) => {
    try {
        ...
        if (_.isNil(_country))
            return res.status(405).send({ statusCode: 405,message:"Please enter valid country"});
        ...
        return res.status(404).send({ statusCode: 404,message:"error Happened ",data: err || "error happened" });
    }
}
