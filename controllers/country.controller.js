...
exports.getCountries = async(req, res) => {
    try {
        ...
        if (!countries)
            return res.status(404).send({ statusCode: 404,message:"Error", data: err || "Try in another time." });
...
exports.updateCountry = async(req, res) => {
    try {
        ...
        if (!countries || countries.length > 1)
            return res.status(404).send({ statusCode: 404,message:"no country found with this data"});
        let _data = req.body;
        ...
        if (_.isNil(updatedCountry) || updatedCountry.length < 1)
            return res.status(404).send({ statusCode: 404,message:"We can not update country.Try in another time."});

        return res.status(200).send({ statusCode: 200,message:"Success", data:updatedCountry});
